...
def nms(cls_detect_array, threshold=0.5):
    ...
    for i in range(len(cls_detect_array)):
        ...
        if cls_detect_array[i][4] < 0.6:  # 置信度小于0.3的都舍弃
            break
    ...
    return cls_detect_array.tolist()


# 对一个文件夹内的所有图片进行相对坐标提取，并使用nms
def one_inference(imgs_dir_path, out_labels_dir_path):
    # 读取配置
...
# 返回所有标签的合并
def outlabels_merge(out_labels_dir_path):
    CLASSES = ('1', '2', '3')
    # 类别反查表
    ...
    label_list = os.listdir(out_labels_dir_path)
    for label_name in label_list:

        label_path = os.path.join(out_labels_dir_path, label_name)
        ...
        for line in lines:
            ...
            classes_boxes[label_info[0]].append(
                [float(label_info[1])+1000, float(label_info[2])+1000, float(label_info[3])+1000, float(label_info[4])+1000,
                 float(label_info[5])])

    with open('../result/merged.json', 'w', encoding='utf-8') as merged:
        json.dump(classes_boxes, merged)
    pprint(classes_boxes)
    return classes_boxes


def test_with_nms(imgs_dir_path, out_labels_dir_path):
    # batch_inference(imgs_dir_path, out_labels_dir_path)

    classes_infos = outlabels_merge(out_labels_dir_path)
    cls1 = nms(np.array(classes_infos['1'], dtype=float))
    cls2 = nms(np.array(classes_infos['2'], dtype=float))
    cls3 = nms(np.array(classes_infos['3'], dtype=float))
    ...
    with open('../result/final.json', 'w', encoding='utf-8') as final:
        json.dump(final_info, final)


test_with_nms('../datasets/test/data', '../result/label')
# merge = open('../result/merged.json','r',encoding='utf-8')
# final = open('../result/all_boxes.json','r',encoding='utf-8')